...
sub getMouseChromosomes
{
        return (1..19, 'M', 'X', 'Y');
}

sub addMouseChromosomes
{
        my ($item_doc, $org_item) = @_;
        my $chr = {
                        map { $_ => $item_doc->add_item('Chromosome', primaryIdentifier => $_, organism => $org_item) } getMouseChromosomes
                };
        return $chr;
}


sub getHumanChromosomes
{
        return (1..22, 'M', 'X', 'Y');
}

sub addHumanChromosomes
{
        my ($item_doc, $org_item) = @_;
        my $chr = {
                        map { $_ => $item_doc->add_item('Chromosome', primaryIdentifier => $_, organism => $org_item) } getHumanChromosomes
                };
        return $chr;
}
