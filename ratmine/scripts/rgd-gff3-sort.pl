...
sub printSortedData
{
	...
	my @order = qw(gene mRNA UTR5 EXON CDS UTR3 SimpleSequenceLengthVariation QTL);
	...
	foreach my $feature (@order)
	{
		foreach my $lines ($$sorted_data{$feature})
		{
			foreach my $line (@$lines)
			{
				...
				$data[2] =~ s/EXON/Exon/g;
				$data[2] =~ s/UTR5/FivePrimeUTR/g;
				$data[2] =~ s/UTR3/ThreePrimeUTR/g;
				$data[8] =~ s/Name=Gene:/Name=/g;
				$data[8] =~ s/ID=RGD/ID=RGD:/g;
				$data[8] =~ s/MGD:MGI/MGD:MGI:/g;
				$data[8] =~ s/=\s+/=/g;
				$data[8] =~ s/,\t/,/g;
				$data[8] =~ s/\s+?;/;/g;
