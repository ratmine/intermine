...
while(<$RGD>)
{
	...
	foreach my $g (split(/\|/, $gene_id)) {
		...
		#print "$g\t$rgd_mapping{$g}{id}\t$rgd_mapping{$g}{symbol}\n";
	}
...
while(<$PKB>)
{
	...
	my @fields = split(/\t/);
   	my %line = zip(@$index, @fields);
	#print %line;
	my $pharm_id = $line{PharmGKB_Accession_Id};
	my $gene_id = $line{Entrez_Id};
	unless (exists($rgd_flags{ $rgd_mapping{$gene_id}{id} }))
	{
		if($rgd_mapping{$gene_id}{id})
		{
			#push(@{$rgd_mapping{$gene_id}{pharmids}}, $pharm_id);
			my $gene_item = $item_doc->add_item('Gene', primaryIdentifier => "RGD:".$rgd_mapping{$gene_id}{id},
